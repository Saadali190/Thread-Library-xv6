...
wait(void)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc || p->pgdir == proc->pgdir || p->is_thread == 1)   // (*) Continue if a there is a thread
        continue;
      havekids = 1;
      if(p->state == ZOMBIE){
        // Found one.
...
clone(void(*fcn)(void *), void *arg, void *stack)
{
  ...
  np->tf->ebp = stack_pointer;
  user_stack[0] = 0xffffffff;
  user_stack[1] = (uint)arg;
  stack_pointer -= 2*sizeof(uint);
  if (copyout(np->pgdir, stack_pointer, user_stack, 2*sizeof(uint)) < 0)
    return -1;
...
int join(void **stack)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->parent != proc || p->pgdir != proc->pgdir || p->is_thread==0)
        continue;
      havekids = 1;
      if(p->state != ZOMBIE)
        continue;
